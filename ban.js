...
exports.run = (bot, message, args, prefix, Discord) => {
  ...
  if(message.content.startsWith(prefix + "ban")) {
    ...
    // ---- Les droits nécéssaire à la commande ---- //
        let myrole = message.guild.member(bot.user).hasPermission("BAN_MEMBERS"); // Récupération des droits nécéssaires du bot
        let yourole = message.guild.member(message.author).hasPermission("BAN_MEMBERS"); // Récupération des droits nécéssaires du membre
        ...
        if(!message.guild.members.get(memberkick.id).Banable) return message.channel.send("L'utilisateur ne peut pas être kick");
        ...
        message.guild.member(memberkick).ban();
        logs.send({
            embed: {
                ...
                fields: [{
                    ...
                    name: "Ban de " + memberkick.tag + " par " + message.author.tag,
                    value: "Raison : " + reason
                }],
              footer: {
